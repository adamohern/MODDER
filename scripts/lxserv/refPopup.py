...
class refPopup(lxifc.UIValueHints):
	def __init__(self, ref_path):
		ref = []
		for (dirpath, dirnames, filenames) in walk(ref_path):
			ref.extend(filenames)

		regex = re.compile(r'^\.')
		ref = [i for i in ref if not regex.search(i)]
		
		regex = re.compile(r'\.ref\.')
		ref = [i for i in ref if regex.search(i)]
...
class refPopupCmd(lxu.command.BasicCommand):
	def __init__(self):
		...
		self.dyna_Add('ref', lx.symbol.sTYPE_STRING)
	...
	def arg_UIValueHints(self, index):
		...
		ref_path = join(kit_path,'scripts')
		if index == 0:
			return refPopup(ref_path)
	...
	def cmd_Execute(self,flags):
		...
		if self.dyna_IsSet(0):
			kit_path  = lx.eval("query platformservice alias ? {kit_mecco_sky_py:}")
			lx.eval('@{%s}' % join(kit_path,'scripts',self.dyna_String(0)))
...
lx.bless(refPopupCmd, "sky.refPopup")